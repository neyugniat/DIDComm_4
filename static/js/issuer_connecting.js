document.getElementById('issuerFetchBtn')?.addEventListener('click', async () => {
  const urlDisplay = document.getElementById('issuerInvitationUrl');
  const qrDisplay = document.getElementById('issuerQRCode');
  const jsonDisplay = document.getElementById('issuerInvitationJson');
  const copyBtn = document.getElementById('issuerCopyBtn');
  ...
  try {
    const response = await fetch('/connections/issuer/create-invitation', {
      method: 'POST',
