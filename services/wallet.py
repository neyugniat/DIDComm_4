...
async def get_credential_revocation_status(credential_id: str):
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get(f"{settings.HOLDER_AGENT_URL}/credential/revoked/{credential_id}")
            response.raise_for_status()
            return response.json()
    except httpx.HTTPStatusError as e:
        logger.error(f"Failed to fetch revocation status for credential {credential_id}: {e.response.status_code} - {e.response.text}")
        raise Exception(f"Failed to fetch revocation status for credential {credential_id}: {e.response.status_code}")
    except Exception as e:
        logger.error(f"Error fetching revocation status for credential {credential_id}: {str(e)}")
        raise Exception(f"Error fetching revocation status for credential {credential_id}: {str(e)}")